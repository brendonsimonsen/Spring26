...
parse\_lable\_with\_rest - validates label names and supports labels appearing ont he same line as instructions.
...
The parse function iterates through every token and reacts based on its type. It builds a ParsedProgram while keeping track of labels that appear before instructions or data
...
**Data declaration** - For data declarations, the parser ensures it’s in the .data section, assigns labels an address in data memory, creates a DataWord structure, and updates the data offset.
...
The helper functions-parse\_register(), parse\_immediate(), calculate\_branch\_offset(), and get\_funct\_code()-handle parsing registers, parsing immediates, calculating branch offsets, and mapping opcodes and function codes. This keeps the main encoding logic clean and readable.
